...
function piwfCreateDbTables($db, $cfg) {
    $sql = 'CREATE TABLE `' . $cfg['sql']['sqlprefix'] . '_piwf_actions` (
                `idworkflowaction` int(10) NOT NULL DEFAULT "0",
                `idworkflowitem` int(10) NOT NULL DEFAULT "0",
                `action` varchar(255) NOT NULL,
                PRIMARY KEY (`idworkflowaction`)
            )';
    $db->query($sql);
    $sql = 'CREATE TABLE `' . $cfg['sql']['sqlprefix'] . '_piwf_allocation` (
                `idallocation` int(10) NOT NULL DEFAULT "0",
                `idworkflow` int(10) NOT NULL DEFAULT "0",
                `idcatlang` int(10) NOT NULL DEFAULT "0",
                PRIMARY KEY (`idallocation`)
            )';
    $db->query($sql);
    $sql = 'CREATE TABLE `' . $cfg['sql']['sqlprefix'] . '_piwf_art_allocation` (
                `idartallocation` int(10) NOT NULL DEFAULT "0",
                `idartlang` int(10) NOT NULL DEFAULT "0",
                `idusersequence` int(10) NOT NULL DEFAULT "0",
                `starttime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                `laststatus` varchar(32) DEFAULT NULL,
                `lastusersequence` int(10) NOT NULL DEFAULT "0",
                PRIMARY KEY (`idartallocation`)
            )';
    $db->query($sql);
    $sql = 'CREATE TABLE `' . $cfg['sql']['sqlprefix'] . '_piwf_items` (
                `idworkflowitem` int(10) NOT NULL DEFAULT "0",
                `idworkflow` int(10) NOT NULL DEFAULT "0",
                `position` int(10) NOT NULL DEFAULT "0",
                `name` varchar(255) NOT NULL DEFAULT "0",
                `description` text NOT NULL,
                `idtask` int(10) NOT NULL DEFAULT "0",
                PRIMARY KEY (`idworkflowitem`)
            )';
    $db->query($sql);
    $sql = 'CREATE TABLE `' . $cfg['sql']['sqlprefix'] . '_piwf_user_sequences` (
                `idusersequence` int(10) NOT NULL DEFAULT "0",
                `idworkflowitem` int(10) NOT NULL DEFAULT "0",
                ...
                `emailnoti` int(10) NOT NULL DEFAULT "0",
                `escalationnoti` int(10) NOT NULL DEFAULT "0",
                PRIMARY KEY (`idusersequence`)
            )';
    $db->query($sql);
    $sql = 'CREATE TABLE `' . $cfg['sql']['sqlprefix'] . '_piwf_workflow` (
                `idworkflow` int(10) NOT NULL DEFAULT "0",
                `idclient` int(10) NOT NULL DEFAULT "0",
                `idlang` int(10) NOT NULL DEFAULT "0",
                `idauthor` varchar(32) NOT NULL,
...
$sql = 'SELECT idworkflow
        FROM ' . $cfg['sql']['sqlprefix'] . '_piwf_workflow
        LIMIT 0, 1';
if (!$db->query($sql)) {
    piwfCreateDbTables($db, $cfg);
}
